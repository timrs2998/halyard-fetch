/**
 * Optional interop with Tether Sync when it shares the vault. Destination
 * folders — and this plugin's `data.json`, which can hold plaintext fallback
 * tokens — are registered with Tether Sync's ignore list, so mirrored
 * machine-generated content is never staged, committed, or caught in a
 * checkout/merge race with Tether Sync's own periodic sync.
 *
 * A structural probe, not a dependency. Tether Sync may be absent, or old
...
export function detectTetherSync(app: unknown): IgnoreRegistrar | null {
	const plugins = (app as { plugins?: { plugins?: Record<string, unknown> } } | null | undefined)?.plugins?.plugins;
	const candidate = plugins?.["tether-sync"] as { registerExternalIgnorePattern?: unknown } | undefined;
...
/** Tether Sync's ignore matcher treats a trailing "/" as "this folder and everything under it" — see its `matchPattern`. */
